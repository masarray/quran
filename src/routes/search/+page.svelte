<script>
	...
	async function getSearchResults(searchQuery) {
		...
		try {
			const response = await fetch(`https://api.kalimat.dev/search?query=${searchQuery}&numResults=50`, {
				headers: { 'x-api-key': kalimatPublicApiKey }
			});
	...
	function updateURL(query) {
		if (query && query.length > 0) goto(`/search?query=${query}`, { replaceState: false });
	}
	...
	function processSearchResults(data) {
		...
		data.forEach((item) => {
			if (item.type === 'quran_verse') {
				...
				if (item.type === 'quran_page') processedId = item.id.replace(/^p/, '');
				else if (item.type === 'quran_juz') processedId = item.id.replace(/^j/, '');

				navigationItems.push({ ...item, processedId });
			}
		});
	...
	function sortVerseKeys(verseKeys) {
		return verseKeys.sort((a, b) => {
			...
			if (chapterA === chapterB) return verseA - verseB;
	...
	function getNavigationLink(item) {
		...
		const linkMap = {
			...
			quran_page: [`Halaman ${item.processedId}`, `${baseUrl}/page?id=${item.processedId}`],
...
<div class="mt-4 space-y-4">
	<div class="flex max-w-xl mx-auto">
		<form on:submit|preventDefault={() => updateSearchQuery(document.getElementById('search-input').value)} class="flex items-center w-full">
			...
			<button type="submit" title="Cari" aria-label="Cari" class="py-4 px-5 rounded-r-3xl items-center border border-theme-accent/20 bg-theme-accent/5"><Search2 size={5} /></button>
		</form>
	</div>
		...
		{:else}
			<div id="search-block">
				<div id="search-results-information" class="text-center text-xs">
					{#if resultsFound}
						<span>Menampilkan {totalResults >= 50 ? 'hingga ' : ''}{totalResults} hasil terkait "{searchQuery}".</span>
				...
				<div id="individual-verses-block" in:fade={{ duration: 300 }}>
					{#key resultKeys}
						{#if resultKeys}<FullVersesDisplay keys={resultKeys.toString()} />{/if}
					{/key}
				</div>
			</div>
		{/if}
